...
from economic_youtube_action_items_skill.result_store import append_daily_result
...
def _collect_urls(settings: Settings, video_urls: list[str], input_file: str | None) -> tuple[list[str], list[str]]:
    ...
    if not deduped:
        raise ValueError(
            "No input videos found. Provide --video-url/--input-file or set EYT_ACTION_TARGET_CHANNELS."
        )
    return deduped, warnings


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="eyt-action-items", description="Economic YouTube action items CLI")
    sub = parser.add_subparsers(dest="command")

    generate = sub.add_parser("generate", help="Generate action items per video")
...
def run_generate(args: argparse.Namespace) -> int:
    ...
    log_path = Path(settings.log_dir) / f"action-items-{date_key}.log"
