...
from economic_youtube_action_items_skill.processor import extract_action_items
...
from economic_youtube_action_items_skill.youtube import fetch_transcript, infer_was_live, parse_video_id
...
def run_pipeline(urls: list[str], settings: Settings) -> BatchResult:
    ...
    for url in urls:
        ...
        if transcript and status in {ProcessingStatus.COMPLETE, ProcessingStatus.PARTIAL}:
            action_items = extract_action_items(transcript, settings.max_items)
